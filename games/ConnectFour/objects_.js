...
Board = function(wh, sizeGrid) {
  ...
  this.paddingTop = 250
	...
	this.newCoin = function(col) {
		if(this.state && !this.won) {
			if(this.grid[col][0] == -1) {
				...
				if(this.checkWindows(this.turn)) {
					this.won = true
	...
	this.simulateGrid = function(col, turn, grid) {
		return(this.checkWindows(turn, this.addCoin(col, this.getRow(col, grid), turn, grid)))
	}

	this.aiPlay = function() {
		if(!this.aiPlayed) {
			this.timer = millis() + 500
			result = this.minimax(this.copyGrid(), 5, -1000000, 1000000, true)
			console.log(result)
			this.aiPlayed = true
		}else {
			//AI has played
	...
	this.is_terminal_node = function(grid) {
		bool = this.checkWindows(this.AIIndex, grid) || this.checkWindows(nextTurn(this.AIIndex), grid) || this.getValidCols(grid).length == 0
		return(bool)
	}
	this.evaluate_window = function(array, turn) {
		score = 0
		opposite = nextTurn(this.AIIndex)
		if(turn == nextTurn(this.AIIndex)) {
			opposite = this.AIIndex
		}
	...
	this.scorePosition = function(board, turn) {
		score = 0
		// Score center column
		center_array = board[Math.floor(this.sizeGrid/2)]
		center_count = center_array.count(turn)
		score += center_count * 3

		score += this.checkWindows(turn, board, true)
		return(score)
	...
	this.minimax = function(board, depth, alpha, beta, maximazingPlayer){
		...
		if (depth == 0 || is_terminal){
			if(is_terminal){
				if(this.checkWindows(this.AIIndex), board) {
					return['None', 1000000000000]
				}else if(this.checkWindows(nextTurn(this.AIIndex), board)) {
					return['None', -1000000000000]
				}else{
					return['None', 0]
				}
		...
		if(maximazingPlayer) {
			...
			for (var col = 0; col < valid_locations.length; col++) {
				...
				res_new = this.minimax(b_copy, depth-1, alpha, beta, false)
				if (res_new[1] > value) {
					value = res_new[1]
					column = col
				}
			...
			for (var col = 0; col < valid_locations.length; col++) {
				...
				res_new = this.minimax(b_copy, depth-1, alpha, beta, true)
				if (res_new[1] < value) {
					value = res_new[1]
					column = col
				}
  ...
  this.update = function () {
	...
	this.checkWindows = function(player, grid = this.grid, score = false) {
		var sc = 0
		//verticalCheck
		for (var j = 0; j<this.sizeGrid-4 ; j++ ){
      for (var i = 0; i<this.sizeGrid; i++){
				if(score) {
					var array = [grid[i][j], grid[i][j+1], grid[i][j+2], grid[i][j+3]]
					sc += this.evaluate_window(array, player)
				}else {
