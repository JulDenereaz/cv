...
Board = function(wh, sizeGrid) {
  ...
  this.paddingTop = 250
	...
	this.aiPlay = function() {
		if(!this.aiPlayed) {
			...
			for (var col = 0; col < this.nCols; col++) {
				//Check if that column is a win move
				if(this.simulateGrid(col, this.AIIndex, this.copyGrid())) {
					this.aiPlayed = true
					result = [col, 100000]
				//AI avoid player to win next turn
				}else {
					if(this.simulateGrid(col, nextTurn(this.AIIndex), this.copyGrid())) {
						this.aiPlayed = true
						result = [col, 100000]
					}
				}
			...
			if(!this.aiPlayed) {
				result = this.minimax(this.copyGrid(), 5, -1000000, 1000000, this.AIIndex)
				this.aiPlayed = true
			}
		}else {
			//AI has played
			if(this.timer < millis()) {
				this.newCoin(result[0])
				this.aiPlayed = false
			}
		}
	...
	this.is_terminal_node = function(grid) {
		bool = this.checkWin(this.AIIndex, grid) || this.checkWin(nextTurn(this.AIIndex), grid) || this.getValidCols(grid).length == 0
		return(bool)
	}
	this.evaluate_window = function(array, turn) {
		score = 0
		...
		if(array.count(opposite) == 3 && array.count(-1) == 1) {
			score -= 300
	...
	this.scorePosition = function(board, turn) {
		score = 0
		// Score center column
		center_array = board[Math.floor(this.nCols/2)]
		...
		for (var col = 0; col < this.nCols -3; col++) {
			for (var row = 0; row < this.nRows; row++) {
				...
				score += this.evaluate_window(row_array, turn)
			}
		}
		for (var col = 0; col < this.nCols; col++) {
			for (var row = 0; row < this.nRows-3; row++) {
				row_array = [board[col][row], board[col][row+1], board[col][row+2], board[col][row+3]]
				score += this.evaluate_window(row_array, turn)
			}
		}
		...
		for (var col = 0; col < this.nCols-3; col++) {
			for (var row = 0; row < this.nRows -3; row++) {
				...
				score += this.evaluate_window(diag_array, turn)
			}
		}
		...
		for (var col = 0; col < this.nCols-3; col++) {
			for (var row = 0; row < this.nRows-3; row++) {
				...
				score += this.evaluate_window(diag_array2, turn)
			}
		}
	...
	this.minimax = function(board, depth, alpha, beta, maximazingPlayer){
		valid_locations = this.getValidCols(board)
		is_terminal = this.is_terminal_node(board)
		if (depth == 0 || is_terminal){
			if(is_terminal){
				if(this.checkWin(this.AIIndex), board) {
					return['None', 20000]
				}else if(this.checkWin(nextTurn(this.AIIndex), board)) {
					return['None', -20000]
				}else{
					return['None', 0]
				}
			}else {
				return['None', this.scorePosition(board, this.AIIndex)]
			}
		}
		if(maximazingPlayer == this.AIIndex) {
			value = -Infinity
			column = valid_locations[Math.floor(Math.random() * valid_locations.length)];
			for (var col = 0; col < valid_locations.length; col++) {
				row = this.getRow(col, board)
				b_copy = this.addCoin(col, row, this.AIIndex, this.copyGrid(board))
				res_new = this.minimax(b_copy, depth-1, alpha, beta, nextTurn(this.AIIndex))
				if (res_new[1] > value) {
					value = res_new[1]
					column = col
				}
			...
			for (var col = 0; col < valid_locations.length; col++) {
				row = this.getRow(col, board)
				b_copy = this.addCoin(col, row, nextTurn(this.AIIndex), this.copyGrid(board))
				res_new = this.minimax(b_copy, depth-1, alpha, beta, this.AIIndex)
				if (res_new[1] < value) {
					value = res_new[1]
					column = col
				}
